...
local function Rebound()
    ...
    -- Updated Rebound model URL
    local rawUrl = "https://raw.githubusercontent.com/DArrcooMiguel/Rebound-OLDEST/main/Rebound%20For%20Hardcore%201.0.rbxm"
    ...
    if GetLastRoom() ~= nil then
        entityPart.CFrame = GetLastRoom().RoomEnd.CFrame
    else
        entityPart.CFrame = currentRooms:FindFirstChild(latestRoom.Value).RoomEnd.CFrame
    end
                ...
                if canSeeTarget(v.Character, 50) and not v.Character:GetAttribute("Hiding") then
                    ...
                    coroutine.wrap(function()
                        ...
                        game.TweenService:Create(
                            gui.Rebound,
                            TweenInfo.new(0.5),
                            {Size = jspos.Size, Position = jspos.Position}
                        ):Play()
                        ...
                        task.spawn(function()
                            ...
                            game.ReplicatedStorage.GameStats[
                                "Player_" .. Plr.Character.Name
                            ].Total.DeathCause.Value = "Rebound"

                            game.ReplicatedStorage.GameStats[
                                "Player_" .. Plr.Character.Name
                            ]["1"].DeathCause.Value = "Rebound"

                            firesignal(
                                game.ReplicatedStorage.Bricks.DeathHint.OnClientEvent,
                                {
                                    "You died to Rebound...",
        ...
        if currentRooms:FindFirstChild(i) then
            ...
            if room and room:FindFirstChild("RoomStart") then
                ...
                local jerk = game.TweenService:Create(
                    ...
                    {CFrame = abc.CFrame + Vector3.new(0, 0.4, 0)}
                )
...
local function SpawnReb()
    ...
    local rebarrival = G.LoadGithubAudio(
